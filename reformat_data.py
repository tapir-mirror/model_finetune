...
def reformat_jsonl(input_file):
    ...
    with open(input_file, "r") as infile, open(output_file, "w") as outfile:
        # Try to read as a single JSON object first
        try:
                    ...
                    for key in content_keys:
                        if key in msg and msg[key] == "":
                            if "tool_calls" in msg:
                                del msg[key]
                                print(
                                    f"Delete empty '{key}' field in tool call message"
                                )
            ...
            # If not a single JSON, try JSONL format
            infile.seek(0)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reformat a JSON/JSONL file.")
    parser.add_argument("file", type=str, help="The input JSON/JSONL file")
